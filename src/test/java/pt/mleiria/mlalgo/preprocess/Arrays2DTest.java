...
public class Arrays2DTest extends TestCase {
    ...
    public void testTrainSplit() {
        ...
        final List<Pair<Double[][], Double[]>> splitter = Arrays2D.trainTestSplit(x, y, 0.5, true);
    ...
    public void testKfold(){
        ...
        assertEquals(150, x.length);
        final int cv = 7;
        final List<Pair<Double[][], Double[]>> lst = Arrays2D.kFold(x, y, cv, true);
        /*
        lst.stream()
	...
	final List<Pair<Double[][], Double[]>> lst = new ArrayList<>();
	lst.add(new Pair<>(x1, y1));
	lst.add(new Pair<>(x2, y2));
	final Pair<Double[][], Double[]> res = Arrays2D.merge(lst, 20, 5);
