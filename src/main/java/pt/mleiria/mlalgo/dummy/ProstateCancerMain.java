...
public class ProstateCancerMain {
    ...
    public static void main(String[] args) {
        ...
        final List<Tuple2<Double[][], Double[]>> lst = loadData();
        ...
        List<Tuple2<Double, Double>> trainScores = new ArrayList<>();
        List<Tuple2<Double, Double>> testScores = new ArrayList<>();
        for (int i = 0; i < lambdas.length; i++) {
            final Ridge ridge = new Ridge(lambdas[i], false);
            ridge.fit(xTrain, yTrain);
            trainScores.add(new Tuple2<>(lambdas[i], ridge.score(xTrain, yTrain)));
            testScores.add(new Tuple2<>(lambdas[i], ridge.score(xTest, yTest)));
     ...
     * @return
     */
    private static List<Tuple2<Double[][], Double[]>> loadData() {
